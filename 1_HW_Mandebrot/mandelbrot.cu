...
__global__ void MandelFunc (float *Zx, float *Zy, float *K, int n_x, int n_y, float dx, float dy, float x_i, float y_i){
...
int main(int argc, char const *argv[])
{
    ...
    for (unsigned int l=0; l < max_iter; l++){
    	MandelFunc<<<G, B>>>(d_Zx, d_Zy, d_k, n_x, n_y, dx, dy, x_i, y_i);
    }
    // save
	for (unsigned int i=0; i<n_x; i++){
    	for (unsigned int j=0; j<n_y; j++){
    		if (h_k[i+j*n_x] < (x_f - x_i + y_f - y_i))
    			h_k[i+j*n_x] = 0;
    		else
    			h_k[i+j*n_x] %= 16;
    	}-
