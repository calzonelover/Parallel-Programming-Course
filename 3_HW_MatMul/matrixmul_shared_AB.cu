...
__global__ void matmul(float *_A, float *_B, float *_C){
	...
	__shared__ float _Bsub[STRIDE_SIZE][STRIDE_SIZE];
	int i = blockIdx.x * blockDim.x + threadIdx.x;
	int j = blockIdx.y * blockDim.y + threadIdx.y;
	if (i < WB && j < HA){
		_Asub[threadIdx.y][threadIdx.x] = _A[j*STRIDE_SIZE+threadIdx.x];
		_Bsub[threadIdx.y][threadIdx.x] = _B[threadIdx.y*WB+i];
		__syncthreads();
		float sumoverpad = 0.0f;
		for (unsigned int dmmy = 0; dmmy < STRIDE_SIZE; dmmy++){
			sumoverpad += _Asub[threadIdx.y][dmmy]*_Bsub[dmmy][threadIdx.x];
		}
